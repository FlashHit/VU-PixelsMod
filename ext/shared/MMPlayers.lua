...
function MMPlayers:Write(mmResources)
	...
	if (mmResources:IsLoaded('yump')) then
		...
		playerYump.jumpHeight = var_yump_heigt
		playerYump.jumpEffectSize = var_yump_size
	...
	if (mmResources:IsLoaded('pose_stand') and
		...
		poseStand.velocity = var_stand_velocity
		poseStand.sprintMultiplier = var_stand_multiplier
		...
		poseStandAir.velocity = var_air_velocity
		poseStandAir.sprintMultiplier = var_air_miltiplier
		...
		poseSwim.velocity = var_swimming_velocity
		...
		poseClimb.velocity = var_climbing_velocity
		poseClimb.sprintMultiplier = var_climbing_multiplier
		print('Changed Player Climb Pose...')

		local poseClimb = CharacterStatePoseInfo(mmResources:GetInstance('pose_chute'))
		poseClimb:MakeWritable()
		poseClimb.velocity = var_chute_velocity
...
function MMPlayers:onLevelLoaded(levelName, gameMode)
	...
	if (table.has(slowSpeedGamemodes, gm)) then
		...
		if (pose_chute ~= nil) then
			...
			poseChute.velocity = var_chute_velocity
